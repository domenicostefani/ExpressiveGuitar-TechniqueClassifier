...
#<redacted ipython line>    get_ipython().system('ln -sf /opt/bin/nvidia-smi /usr/bin/nvidia-smi')
    None
#<redacted ipython line>    get_ipython().system('pip install gputil')
    None
#<redacted ipython line>    get_ipython().system('pip install psutil')
    None
#<redacted ipython line>    get_ipython().system('pip install humanize')
    None
    import psutil
    import humanize
    import os
...
#<redacted ipython line>from IPython.core.magic import register_cell_magic
None

#<redacted decorator line>@register_cell_magic
None
def write_and_run(line, cell):
    ...
    with open(file, mode) as f:
        f.write(cell)
#<redacted ipython line>    get_ipython().run_cell(cell)
    None
...
with open('feature_selection.txt','a') as fsfile:
    fsfile.write('Selected features: '+str(selected_features))
...
def save_model_info(model,optimizer,final_cross_validation_results,folds,metrics,outpath, fold_zerobased = None):
    ...
    with open(outpath + info_filename, "w") as f:
        ...
        if final_cross_validation_results:
            f.write("Trained for " + str(train_epochs) + " and with_batch size '" + str(train_batch) + "'" + " epochs for each fold ("+str(folds)+"-foldCrossValidation)\n")
            f.write("Single results in the folds directories\n")
            f.write('\n\n-------- Average results --------\n\n')
        else:
            f.write("Trained for " + str(train_epochs) + " and with_batch size '" + str(train_batch) + "'" + " epochs\n")
...
#<redacted ipython line>get_ipython().system('rm -rf ./logs # Clear logs if necessary')
None
#<redacted ipython line>get_ipython().system('sleep 3')
None
...
#<redacted ipython line>    get_ipython().system('pip install tensorflow_model_optimization')
    None
...
os.makedirs(os.path.join(MODELFOLDER,'todelete'),exist_ok = True)
...
for dirpattern, newprefix in [('SingleRun*','acc'),('CrossValidated*','c_acc')]:
    for resdir in glob(os.path.join(MODELFOLDER,dirpattern)):
        ...
        if len(filesindir) == 0:
            ...
            if not os.path.exists(os.path.join(resdir,'info.txt')):
                # raise Exception('info file does not exist in folder "'+resdir+'"')
                os.rename(resdir,os.path.join(MODELFOLDER,'todelete',os.path.basename(resdir)))
